...
public class FileUploadClientHandler extends SimpleChannelInboundHandler<FileUploadEntity> {
    ...
    private volatile long start = 0;//数据读取的起始位置
    private volatile long dataLength = 0;//需读取的数据长度
    ...
    private final int dataGrameNum = 100;//数据段数
    private long dataGrameLength = 0;//数据段数长度
    ...
    public FileUploadClientHandler(FileUploadEntity ef) throws IOException {
        ...
        dataGrameLength = (long) (randomAccessFile.length() / dataGrameNum);
    }
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        ...
        try {
            ...
            //lastLength = 11
            if(dataGrameLength < 0) {
                dataLength = (0 - dataGrameLength);
            }
            else{
                dataLength = dataGrameLength;
            }//分段发送，分11段，115/10=11余数5 10段11 最后一段5
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof Long) {
            start = (Long) msg;//读取到服务端修改后的初始值，第一次读取到start = 11
            System.out.println("服务端传回的指针=》" + start);
            if (start != -1) {
                randomAccessFile.seek(start);//设置文件指针
                long leaveDataLength = (long) (randomAccessFile.length() - start);//115 -11 =104
                //    a=33-0=33    b=33/10=3
    ...
    private boolean upload(ChannelHandlerContext ctx) throws IOException {
        ...
        byte[] bytes = new byte[Math.toIntExact(dataLength)];
